...
def _parse_chat_response(raw: str) -> dict[str, Any]:
    ...
    if not chunks:
        raise ValueError("Response is not JSON or SSE JSON chunks")
...
def _request_chat_completions(
    *,
    ...
    system = (
        ...
        "Prefer returning a JSON object with keys content and sources. "
        "If JSON is not possible, return a concise evidence-backed plain-text answer with source URLs."
    )
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) CodexGrokSearch/1.0",
    }
